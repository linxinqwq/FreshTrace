          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'product'">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div class="text-sm opacity-50">{{ record.senders }}</div>
                  </div>
                </div>
              </template>
              ...
              <template v-else-if="column.key === 'status'">
                <span :class="[
                  'status-text',
                  record.status === 0 ? 'status-pending' : 'status-approved'
                ]">
...
export default {
  ...
  created() {
    ...
    const hide = message.loading('正在获取订单数据...', 0)
    this.$axios.get("/main/get_all_order", {
      params: {
        page: this.page
      }
    }).then((res) => {
      ...
      if (res.data.code == 200) {
        ...
        message.error("获取订单列表失败: " + res.data.message)
      }
    }).catch((err) => {
      hide()
  ...
  methods: {
    ...
    checkOrder(id) {
      const hide = message.loading('正在审核订单...', 0)
      this.$axios.post("/main/check_order", {
        id: id
      }).then((res) => {
        hide()
        if (res.data.code == 200) {
          message.success("审核订单成功")
          const hideReload = message.loading('正在刷新订单数据...', 0)
          this.$axios.get("/main/get_all_order", {
            params: {
              page: this.page,
          ...
          message.error("审核订单失败: " + res.data.message)
        }
      }).catch((err) => {
        hide()
        console.error('审核订单失败:', err)
        message.error("审核订单失败，请稍后重试")
      })
    },
